	...
	<AdminTopbar midText={'Képek'} input={input} modal={kepekModal}></AdminTopbar>
		...
		<form bind:this={input} enctype="multipart/form-data" action="?/foto" method="POST" class="w-full flex gap-5 flex-wrap justify-center mb-5">
			{#each data.termekekLista as termek, i (i)}
				...
				<div class="flex flex-col justify-center w-44 text-center font-semibold">
					<p class="py-1 px-2 bg-primary-container rounded-t-2xl text-on-primary-container">{termek.termek}</p>

					<label class="cursor-pointer rounded-b-2xl overflow-hidden">
						<div class="relative">
							<img class="w-full h-full brightness-50" id="{termek.id}" src="/api/kepek/{termek.id}" alt="">
						...
						<input on:input={(e) => {document.getElementById(termek.id).src = URL.createObjectURL(e.target.files[0]); }} hidden type="file" name="{termek.id}" value="" accept="image/*">
					</label>
				</div>
