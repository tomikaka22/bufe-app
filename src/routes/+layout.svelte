<script>
   ...
   import { cart, total } from '$lib/stores/Cart.js';
	...
	const hideNavbarRoutes = [ '/', '/login', '/register', '/admin', '/offline', '/verify', '/verify/verified', '/banned', '/banned/register', '/install', '/install/app' ];
...
{#if !hideNavbarRoutes.includes($page.url.pathname)}
	<div class:navbar-background-noconflict={!backgroundConflict.includes($page.url.pathname)} class="bg-foreground text-on-background grid grid-cols-4 gap-8 justify-evenly justify-items-center fixed bottom-0 w-full text-sm h-[5.25rem] transition-all duration-200 px-5 pt-3 z-10">
		...
		<a href="/list" class="flex flex-col justify-center items-center h-16 w-full transition-all" class:active-nav={termekekRoutes.includes($page.route.id)}>
			<div class="w-1/2 h-1/2 flex justify-center items-center rounded-3xl py-1" class:active-img={termekekRoutes.includes($page.route.id)} style="transition-timing-function: cubic-bezier(0.05, 0.7, 0.1, 1.0);">
		...
		<a href="/kosar" class="flex flex-col justify-center items-center h-16 w-full transition-all relative" class:active-nav={kosarRoutes.includes($page.route.id)}>
			<div class="w-1/2 h-1/2 flex justify-center items-center rounded-3xl py-1" class:active-img={kosarRoutes.includes($page.route.id)} style="transition-timing-function: cubic-bezier(0.05, 0.7, 0.1, 1.0);">
		...
		<a href="/profil" class="flex flex-col justify-center items-center h-16 w-full transition-all" class:active-nav={profilRoutes.includes($page.route.id)}>
			<div class="w-1/2 h-1/2 flex justify-center items-center rounded-3xl py-1" class:active-img={profilRoutes.includes($page.route.id)} style="transition-timing-function: cubic-bezier(0.05, 0.7, 0.1, 1.0);">
