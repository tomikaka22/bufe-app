...
<main in:fade={{ duration: 180 }}>
	<div class="bg-secondary-container bg-opacity-50 w-full h-14 rounded-b-full flex items-center justify-between px-8 py-1">
		...
		<div class="h-full flex justify-center items-center gap-2">
			<img class="h-full object-contain py-1.5" src="Logo.svg" alt="">
	...
	<Splide on:move={e => { slideIndex = e.detail.index; }} class="mt-2.5 xl:bg-foreground xl:rounded-full xl:overflow-hidden xl:mx-10 xl:py-2" options={{ arrows: false, pagination: false, perPage: 3, focus: 'center', autoWidth: true, autoplay: true, interval: 4000, gap: '2rem', trimSpace: false, rewind: true, start: 1, easing: 'cubic-bezier(0.2, 0.0, 0, 1.0)', flickPower: 1 }}>
		<SplideSlide>
