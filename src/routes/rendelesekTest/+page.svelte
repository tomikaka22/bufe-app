		...
		{#each data.elozmenyLista as record, i (record.id)}
			...
			<div class="mt-5">
				{#if record.status === 'fuggoben'}
					<h3 class="text-center mb-2 font-semibold">Függőben</h3>
				{:else if record.status === 'torolve'}
					<h3 class="text-center mb-2 brightness-50 line-through font-semibold">Törölve</h3>
				{:else if record.status === 'folyamatban'}
					<h3 class="text-center mb-2 font-semibold text-primary">Átvehető!</h3>
				{:else}
					<h3 class="text-center mb-2 font-semibold">{record.updated.slice(0, -5)}</h3>
				{/if}
